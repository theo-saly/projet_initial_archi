...
function isMailEnabled(): boolean {
    return Boolean(SMTP_HOST && SMTP_USER && SMTP_PASS && SMTP_FROM && SMTP_RECIPIENT);
}
...
function buildHtml(event: Record<string, string>): string {
        ...
        const lines: string[] = [];
        if (event.projectId) lines.push(`<li><strong>Projet:</strong> ${escapeHtml(event.projectId)}</li>`);
        if (event.taskId) lines.push(`<li><strong>Tache:</strong> ${escapeHtml(event.taskId)}</li>`);
...
async function lookupEmailByUserId(userId: string): Promise<string> {
    ...
    const payload = await fetchJson(`${AUTH_SERVICE_URL}/auth/users/${encodeURIComponent(String(parsed))}`);
    const email = typeof payload?.email === 'string' ? payload.email : '';
    return email;
}

async function lookupOwnerIdByProjectId(projectId: string): Promise<string> {
    const payload = await fetchJson(`${PROJECT_SERVICE_URL}/projects/${encodeURIComponent(projectId)}`);
    const ownerId = typeof payload?.ownerId === 'string' ? payload.ownerId : '';
    return ownerId;
}

async function resolveRecipient(event: Record<string, string>): Promise<string> {
...
export async function sendEmailNotification(event: Record<string, string>): Promise<void> {
    if (!transporter) {
        return;
    }
    ...
    if (!to) {
        console.warn('SMTP: aucun destinataire disponible pour l\'evenement', event.eventType);
        return;
    }
