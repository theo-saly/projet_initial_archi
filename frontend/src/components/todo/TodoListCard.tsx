...
interface TodoListCardProps {
    ...
    createTask: (payload: { title: string; description: string; status: string }) => Promise<unknown>;
...
export default function TodoListCard({
    ...
    const [taskForm, setTaskForm] = useState({ title: '', description: '', status: 'à faire' });
    const [editingTaskId, setEditingTaskId] = useState<string | null>(null);
    const [editTaskForm, setEditTaskForm] = useState({ title: '', description: '' });

    const safeTasks = Array.isArray(tasks) ? tasks : [];
    const completeCount = safeTasks.filter((t) => t.status === 'terminé').length;
    ...
    const onCreateTask = async (event: React.FormEvent) => {
        ...
        setBusy(true);
        try {
            await createTask({ title: taskForm.title.trim(), description: taskForm.description.trim(), status: taskForm.status });
    ...
    const onSaveEditTask = async (task: Task) => {
        ...
        setBusy(true);
        try {
            await updateTask(task.id, { title: nextTitle, description: nextDescription, status: task.status });
            onCancelEditTask();
    ...
    return (
        <section className="card">
            ...
            <div className="card-body">
                ...
                {safeTasks.length === 0 && (
                    <p className="text-muted mb-0">Aucune tache pour ce projet.</p>
                )}
